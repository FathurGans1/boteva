...
module.exports = {
  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            let ppgc = 'https://telegra.ph/file/99fc5c91c8dfcdc58a557.jpg'
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                (chat.sBye || this.bye || conn.bye || 'GoodBye, @user!')).replace(/@user/g, '@' + user.split`@`[0])
      ...
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user not admin now')
